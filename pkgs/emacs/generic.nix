...
, Xaw3d, libXcursor,  pkg-config, gettext, libXft, dbus, libpng, libjpeg, giflib, tree-sitter
...
assert withTreeSitter -> lib.versionAtLeast version "29";
...
(if withMacport then llvmPackages_6.stdenv else stdenv).mkDerivation (finalAttrs: (lib.optionalAttrs nativeComp {
  ...
  src = if macportVersion != null then fetchFromBitbucket {
    ...
    repo = "emacs";
    rev = if rev != "" then rev else version;
    inherit sha256;
  };
